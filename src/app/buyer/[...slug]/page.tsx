...
export default function BuyerSlugPage({ params }: { params: Promise<{ slug?: string[] }> }) {
  const resolvedParams = use(params);
  const slug = resolvedParams.slug?.join("/") ?? "";
  if (!slug) return <RoleHome role="buyer" spec={ROLE_HOME_SPEC.buyer} />;
  return <RoleModulePage role="buyer" slug={slug} />;
}
